...
# def random_split_chem(dataset, task_idx=None, null_value=0,
#                    frac_train=0.8, frac_valid=0.1, frac_test=0.1, seed=0,
#                  smiles_list=None):
#     """
...
## For Bio dataset
def random_split(idx_for_split, frac_train=0.8, frac_test=0.1,
                 seed=0):
    """
    Adapted from graph-pretrain
    :param idx_for_split: indeces for split
    :return: train, test indeces of the input idx.
    """
    num_samples = len(idx_for_split)
    train_test_idx_idx = torch.randperm(len(idx_for_split))
    num_train = int(num_samples*0.85)
    num_test = num_samples - num_train
    train_idx = idx_for_split[train_test_idx_idx[:num_train]]
    test_idx = idx_for_split[train_test_idx_idx[num_train:]]
...
def species_split(dataset, train_valid_species_id_list=[3702, 6239, 511145,
    ...
    train_valid_byte_tensor = torch.zeros(len(dataset), dtype=bool)
    for id in train_valid_species_id_list:
        train_valid_byte_tensor += (dataset.data.species_id == id)

    test_species_byte_tensor = torch.zeros(len(dataset), dtype=bool)
    for id in test_species_id_list:
        test_species_byte_tensor += (dataset.data.species_id == id)

    return train_valid_byte_tensor, test_species_byte_tensor
