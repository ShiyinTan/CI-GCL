...
from GCL.utils import compute_infonce, cluster_get, CustomDataLoader, compute_cluster_constrain_loss, k_fold, process_degree, process_topo_eigens, save_model_evaluator, \
...
def get_config():
    ...
    parser.add_argument("--spectral_topo_weight", type=float, default=0.8)
    parser.add_argument("--spectral_feature_weight", type=float, default=0.3)
    

    parser.add_argument("--mode", type=str, default="unsup", choices=["unsup", "semisup"])
    parser.add_argument("--dataset_name", type=str, default="Synthetic_Graphs")
...
def train(encoder_model, predictor, dataloader, optimizer, args, augs_type=None, device='cpu'):
    ...
    for data, extra_info_dict in dataloader:
        ...
        loss = ssl_loss + predict_loss + args.spectral_topo_weight*spectral_topo_loss + args.spectral_feature_weight*spectral_feature_loss
...
def test(encoder_model, dataloader, args, split, epoch_select="val_max", best_evaluator=None, device='cpu'):
    ...
    with warnings.catch_warnings():
        ...
        if best_evaluator==None:
            result, best_evaluator = SVMEvaluator(linear=args.linear, epoch_select=epoch_select)(x, y, split)
        else:
...
def main():
    ...
    dataset = torch.load(osp.join(path, dataset_name, "MySavedGraph.pt"))
    ...
    for fold_i, (train_idx, val_idx, test_idx) in enumerate(k_fold(len(dataset), epoch_select=args.epoch_select)):
        ...
        test_result, best_evaluator = test(best_encoder_model, dataloader, args, split, epoch_select=args.epoch_select, best_evaluator=None, device=device) # only for NCI1, PROTEINS
        all_fold_results['micro_f1'].append(test_result['micro_f1'])
        all_fold_results['macro_f1'].append(test_result['macro_f1'])
