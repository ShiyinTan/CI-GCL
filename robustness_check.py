...
def get_config():
    ...
    parser.add_argument("--mode", type=str, default="robust", choices=["unsup", "semisup", "robust"])
...
def robust_test(encoder_model, dataloader, args, split, noise_rate=0.04, best_evaluator=None, device='cpu'):
    ...
    for data, _ in dataloader:
        ...
        if noise_rate==0:
            ...
            num_sample_edges = int(0.5 * noise_rate * data.edge_index.shape[1])//len(data)
            sampled_edges = batched_negative_sampling(data.edge_index, batch=data.batch, num_neg_samples=num_sample_edges).long()
            new_edge_index, edge_id = dropout_edge(data.edge_index, p=(0.5*noise_rate), force_undirected=True)
            new_edge_index = torch.cat([new_edge_index, sampled_edges], dim=1)

        edge_weights = torch.ones((new_edge_index.shape[1], 1), device=device)

        z, g = encoder_model(data.x, new_edge_index, data.batch, edge_weights=edge_weights, mode="eval")
        x.append(g)
        y.append(data.y)
    x = torch.cat(x, dim=0)
    y = torch.cat(y, dim=0)
...
def main():
    ...
    for noise_rate in [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]:
