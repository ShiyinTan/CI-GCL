...
class ResGCN(nn.Module):
    ...
    def __init__(self, dataset, hidden, num_feat_layers=1, num_conv_layers=3,
        ...
        self.project = nn.Sequential(nn.Linear(hidden, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, hidden))
    ...
    def forward(self, x, edge_index, batch, edge_weight=None, **kwargs):
        ...
        if self.res_branch == "BNConvReLU":
            ...
            return self.forward_BNConvReLU_without_pred(x, edge_index, batch, xg=xg, edge_weight=edge_weight)
        else:
    ...
    def forward_BNConvReLU_without_pred(self, x, edge_index, batch, edge_weight=None, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index))
        for i, conv in enumerate(self.convs):
            x_ = self.bns_conv[i](x)
            ...
            x = x + x_ if self.conv_residual else x_
        gate = 1 if self.gating is None else self.gating(x)
        g = self.global_pool(x * gate, batch)
        g = g if xg is None else x + xg
        for i, lin in enumerate(self.lins):
            g_ = self.bns_fc[i](g)
            g_ = F.relu(lin(g_))
            g = g + g_ if self.fc_residual else g_
        g = self.bn_hidden(g)
        if self.dropout > 0:
