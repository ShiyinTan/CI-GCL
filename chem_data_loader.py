...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if not empty:
            self.data, self.slices = torch.load(self.processed_paths[0], map_location=map_device)
            self.data = Data.from_dict(self.data.__dict__)
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        ...
        root = os.path.join(os.path.expanduser('~'), 'datasets/chem_dataset', dataset_name)
